...
class Topology(object):
    ...
    def createTopology(self):
        ...
        if self.settings.topology :
            topo = open("topology"+str(len(self.motes))+".txt",self.settings.rw)
        # reposition each mote until it is connected
        connectedMotes = [dagRoot]

        
        if self.settings.topology and self.settings.rw == 'r':
            lines = []
            for line in topo :
                lines += [line]
            if lines :
                ...
                for mote in self.motes :
                    ...
                    while not connected :
                        ...
                        mote.setLocation(float(coord[0]),float(coord[1]))
                        numStableNeighbors = 0
                        j = 0

                        for cm in (c for c in connectedMotes if c.id != mote.id):
        ...
        for mote in self.motes:
            if mote in connectedMotes or self.settings.rw == "r":
                continue
            connected = False
            while not connected:
                mote.setLocation(
        ...
        for mote in self.motes:
            if self.settings.topology and self.settings.rw == "w":
                (x,y) = mote.getLocation()
                topo.write(str(x) + " "+str(y) + " ")
            for m in self.motes:
                if mote==m:
                    continue
                if self.settings.topology and self.settings.rw == "w":
                    topo.write(str(self._computeRSSI(mote, m)) + " ")

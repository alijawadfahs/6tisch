...
class SimEngine(threading.Thread):
    ...
    def getNextTS(self, sharedCellList, dir):
        (ts, ch) = self.lastAllocated
        if ts == self.settings.slotframeLength :
            ch = ch +1
            ts = 1
        else :
            ts = ts + 1
        while ch == 0 and ts in sharedCellList :
            ts = ts + 1
        self.lastAllocated = (ts, ch)
        return (ts, ch, dir)
            ...
            with self.dataLock:
                ...
                while True:
                    if self.events[0][0]!=self.asn:
                        break
                    (_,prio,cb,_,args) = self.events.pop(0)
                    cb(args)

        # call the end callbacks
        for cb in self.endCb:
            cb()
        print "Network needed " + str(self.cellNeeded) + " cells"
        # log
    ...
    def scheduleIn(self,delay,cb,uniqueTag=None,priority=0,exceptCurrentASN=True, args=None):
        ''' used to generate events. Puts an event to the queue '''
        with self.dataLock:
            asn = int(self.asn+(float(delay)/float(self.settings.slotDuration)))
            self.scheduleAtAsn(asn,cb,uniqueTag,priority,exceptCurrentASN, args)

    def scheduleAtAsn(self,asn,cb,uniqueTag=None,priority=0,exceptCurrentASN=True,args=None):
        ...
        with self.dataLock:
            ...
            # add to schedule
            self.events.insert(i,(asn,priority,cb,uniqueTag, args))
    ...
    def _actionEndSim(self, args=None):
        with self.dataLock:
            self.goOn = False
